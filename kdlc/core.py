...
def normalize_connections(
    ...
    for connection in connection_list:
        ...
        if isinstance(connection.source_node, MetaNode) and not isinstance(
            connection.source_node, WrappedMetaNode
        ):
            connection.source_port = str(int(connection.source_port) - 1)
        if isinstance(connection.dest_node, MetaNode) and not isinstance(
            connection.dest_node, WrappedMetaNode
        ):
